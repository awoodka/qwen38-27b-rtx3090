...
| `REASONING_EFFORT` | empty | (fork) empty serves the model's own chat template. `xhigh`, `focused`, `medium` or `low` serves `templates/qwen3.8-27b.jinja` with that level as the server's default: the model's template plus a `focused` level, which keeps xhigh's depth and asks for less re-verification, and `max`/`high` and `minimal`/`none` taken as xhigh and low instead of a 400. It renders the same bytes as the model's template for every input that template accepts, so `xhigh` serves what empty does. A request's own `reasoning_effort`, top-level or in `chat_template_kwargs` (the only way to ask for `focused` per request), still wins. Anything else refuses to start. See `docs/thinking-levers.md` |
| `THINK_PENALTY` | 0 | (fork) logits subtracted, before temperature, from the tokens of `THINK_PENALTY_WORDS` while the model is inside its reasoning (`patches/think-penalty.patch`, passed as `--reasoning-config`). 0 is off; at most 100, which is a ban in practice. The model's reply after `</think>` is never penalized, nor is anything with thinking off. `SPEC=dflash2` only: the patch lives in Model Runner V2, and the launcher refuses V1 configurations. The server logs `Think penalty: -3.00 logits on 6 token ids` when it is on. See `docs/thinking-levers.md` |
| `THINK_PENALTY_WORDS` | `Wait,Hmm,Alternatively` | (fork) the comma-separated words `THINK_PENALTY` applies to, each as written and with one leading space. A form that is not exactly one ordinary token is dropped and logged, so lowercase `hmm` keeps only ` hmm`; `wait` is left out of the default because code says `.wait(` |
